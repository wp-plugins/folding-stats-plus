...
add_option('folding_results_team', 'true');
...
if ($_POST['fold_acct'] == '') { 
 ...
 //  update_option('folding_align', $_POST['fold_align']);
  update_option('folding_results_color', $_POST['fold_results_color']);
		...
		if (isset($_POST['fold_results_team'])) {
		update_option('folding_results_team', 'true');
		} else {
			update_option('folding_results_team', 'false');
		}
...
$fold_results_team = get_option('folding_results_team');
?>
<form name="fold"></form>
<div class="wrap">
...
<div class="inside">
  		<p><label  for="fold_acct">Account ID:</label><input style="width: 80px;" id="fold_acct" name="fold_acct" type="text" value="<?php echo $fold_acct;?>" /><? if ($fold_acct == 'fold-id') { echo '<font style="color: #ff0000;">Set your FAH name!</font>'; } ?><br />
		    ...
		    <label for="fold_results_team">link to team?:</label>
			<input id="fold_results_team" name="fold_results_team" type="checkbox" value="<?php print get_option('folding_results_team');?>" <?php if(get_option('folding_results_team') == 'true') {?> checked="checked" <?php } ?>/><br />		
		    		    <br />
			Next update run on: <?php echo date("F j, Y, g:i a",get_option('folding_expire')); ?> 
			<br /> Server time: <?php echo date("F j, Y, g:i a",time()); ?>
